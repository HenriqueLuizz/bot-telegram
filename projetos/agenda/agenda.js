...
const { getAgenda, getTarefa, getTarefas, getConcluidas, incluirTarefa,
    concluirTarefa, excluirTarefa, atualizarDataTarefa, atualizarObsTarefa } = require('./agendaServicos')
...
bot.command('amanha', async ctx => {
    ...
    ctx.reply(`Aqui está a sua agend até amanhã`, botoesAgenda(tarefas))
})
...
bot.action(/concluir (.+)/, async ctx => {
    ...
    await exibirTarefa(ctx, ctx.match[1])
    await ctx.reply(`Tarefa Concluida`)
})

bot.action(/excluir (.+)/, async ctx => {
    await excluirTarefa(ctx.match[1])
    await ctx.editMessageText('Tarefa Excluida')
})
...
dataScene.hears(/(\d{2}\/\d{2}\/\d{4})/g, async cxt => {
...
obsScene.on('text', async ctx => {
    ...
    const resp = await atualizarObsTarefa(idTarefa, obs)
    await ctx.reply('Observação adicionada!')
    await exibirTarefa(ctx, idTarefa, true)
    ctx.scene.leave()
})

obsScene.on('message', ctx => ctx.reply('Apenas observações em texto são aceitos.'))
